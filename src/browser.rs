...
use wasm_bindgen::{
    closure::{IntoWasmClosure, WasmClosure, WasmClosureFnOnce},
    prelude::*,
    JsCast, JsValue,
};
...
pub async fn fetch_json(json_path: &str) -> Result<JsValue> {
    ...
    .await
    .map_err(|err| anyhow!("Error fetch JSON {:#?}", err))
}
...
pub fn closure_wrap<T, F>(f: F) -> Closure<T>
where
    T: ?Sized + WasmClosure,
    F: 'static + IntoWasmClosure<T>,
{
    Closure::new(f)
}
